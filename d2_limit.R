...
g <- exp(-pi^2*cohlen^2/ln2*f^2)
#g <- dnorm(f, 0, 0.1)
c <- 1-(conv(g,p)/trapz(seq(0, count/2, length.out= length(p)), p))
print(length(c))
...
for (freq in freqs) {
  ...
  print(paste('num/den:', toString(intTerm1), '/', toString(intNorm)))
